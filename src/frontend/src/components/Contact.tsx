...
export default function Contact() {
  ...
  const address =
    settings?.businessAddress ?? t(translations.contact.addressValue, lang);
  const hours =
    settings?.businessHours ?? t(translations.contact.hoursValue, lang);
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="space-y-4">
            {contactItems.map((item, i) => (
              <div
                key={item.label}
                className="bg-card border border-border rounded-2xl p-5 flex items-start gap-4 card-hover"
              >
                <div
