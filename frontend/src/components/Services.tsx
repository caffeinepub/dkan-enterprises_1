import { Wind, Refrigerator, WashingMachine, Tv, Flame, Droplets, Microwave } from 'lucide-react';
...
function getServiceIcon(name: string, category: string) {
  ...
  if (lower.includes('washing') || lower.includes('washer')) return WashingMachine;
  if (lower.includes('tv') || lower.includes('lcd') || lower.includes('led')) return Tv;
  if (lower.includes('geyser') || lower.includes('heater')) return Flame;
  if (lower.includes('water') || lower.includes('purifier') || lower.includes('ro')) return Droplets;
...
export default function Services() {
  ...
  const displayServices: DisplayService[] = (backendServices && backendServices.length > 0)
            ...
            {displayServices.length === 0 ? (
              <div className="text-center py-16 text-muted-foreground">
                ...
                <p className="text-base font-medium">
                  {lang === 'hi' ? 'अभी कोई सर्विस उपलब्ध नहीं है' : 'No services available at the moment'}
                </p>
                <p className="text-sm mt-1">
                  {lang === 'hi' ? 'कृपया बाद में दोबारा देखें' : 'Please check back later'}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayServices.map((service, i) => {
                  ...
                  const displayName = lang === 'hi' && service.nameHindi ? service.nameHindi : service.name;
                  const displayDesc = lang === 'hi' && service.descriptionHindi ? service.descriptionHindi : service.description;

                  return (
                    <div
                      key={service.id}
                      className={`group relative bg-gradient-to-br ${serviceColors[colorIdx]} border rounded-2xl p-6 card-hover cursor-pointer`}
